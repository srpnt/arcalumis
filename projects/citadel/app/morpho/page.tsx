...
function VaultBarTooltip({ active, payload }: BarTooltipProps) {
...
export default function MorphoPage() {
  ...
  const scatterData = useMemo(() => {
    return filtered
      ...
      .map((v) => ({
        name: v.name,
        tvl: v.totalAssetsUsd,
        apy: +(v.netApy * 100).toFixed(2),
        chain: v.chain,
        chainId: v.chainId,
        markets: v.numMarkets,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: Record<string, unknown>) => {
        const v = row as unknown as MorphoVault;
        return <ChainBadge chain={v.chain} />;
      },
    },
    {
      key: "underlyingAsset",
      ...
      render: (row: Record<string, unknown>) => {
        const v = row as unknown as MorphoVault;
        return <span className="text-gray-300">{v.underlyingAsset}</span>;
      },
    },
    {
      key: "totalAssetsUsd",
      ...
      render: (row: Record<string, unknown>) => {
        const v = row as unknown as MorphoVault;
        return <span className="font-mono text-gray-200">{formatUsd(v.totalAssetsUsd)}</span>;
      },
    },
    {
      key: "netApy",
      ...
      render: (row: Record<string, unknown>) => {
        ...
        return (
          <span className="font-mono" style={{ color: `rgb(${255 - green}, ${100 + green}, 100)` }}>
            {formatPct(v.netApy)}
          </span>
        );
      },
    },
    {
      ...
      render: (row: Record<string, unknown>) => {
        const v = row as unknown as MorphoVault;
        return <span className="text-gray-400 font-mono">{formatPct(v.apy)}</span>;
      },
    },
    {
      key: "fee",
      ...
      render: (row: Record<string, unknown>) => {
        const v = row as unknown as MorphoVault;
        return <span className="text-gray-500 font-mono">{formatPct(v.fee, 0)}</span>;
      },
    },
    {
      key: "numMarkets",
      ...
      render: (row: Record<string, unknown>) => {
        const v = row as unknown as MorphoVault;
        return <span className="font-mono text-gray-300">{v.numMarkets}</span>;
      },
    },
  ];
