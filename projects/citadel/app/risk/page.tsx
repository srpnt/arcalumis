...
type SignalFilter = "all" | "critical" | "notable" | "info";

const SIGNAL_TABS: { key: SignalFilter; label: string; icon: string }[] = [
...
export default function RiskPage() {
  ...
  // Exposure data
  const { data: exposureData, error: exposureError, isLoading: exposureLoading, mutate: mutateExposure } = useSWR(
    "collateral-exposure",
    fetchExposure,
  ...
  // Signals data
  const { data: signalsData, error: signalsError, isLoading: signalsLoading } = useSWR(
    "signals",
    fetchSignals,
    { revalidateOnFocus: false, refreshInterval: 43_200_000 }
  );

  const assets = exposureData?.assets || [];
  ...
  const totalExposure = exposureData?.totalExposureUsd || 0;
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: Record<string, unknown>) => {
        const a = row as unknown as CollateralAsset;
        return <RiskTierBadge tier={a.tier} label={a.tierLabel} emoji={a.tierEmoji} />;
      },
    },
    {
    ...
    <div className="pt-8 md:pt-0">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Collateral exposure and risk alerts
          </p>
        </div>
      </div>

      {/* Main Tabs: Exposure / Alerts */}
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            <MetricCard
              ...
              value={String(exposureData?.totalAssetsTracked || 0)}
              icon="🛡"
            />
            <MetricCard
              label="Tier 4 (Exotic) Exposure"
              value={formatUsd(exposureData?.tier4ExposureUsd || 0)}
            ...
            <MetricCard
              ...
              value={
                exposureData?.highestConcentration
                  ? `${exposureData.highestConcentration.symbol} — ${(exposureData.highestConcentration.pct * 100).toFixed(1)}%`
                  : "—"
              }
          ...
          {!exposureLoading && assets.length > 0 && (
          ...
          {/* Empty state */}
          {!exposureLoading && !exposureError && rows.length === 0 && exposureData && (
            <div className="text-center py-12 text-gray-500">
      ...
      {activeTab === "alerts" && (
        <>
          ...
          {/* Empty State */}
          {filteredSignals.length === 0 && !signalsLoading && !signalsError && (
            <div className="text-center py-16 text-gray-600">
              <p className="text-4xl mb-4">📡</p>
              <p className="text-sm">
                {signalFilter === "all"
                  ? "No signals detected yet. Check back after the next research scan."
                  : `No ${signalFilter} signals. Adjust filters to see more.`}
              </p>
            </div>
          )}
